...
AMC::FeatureExtractor::FeatureExtractor(boost::shared_ptr<SharedBuffer<std::complex<double> > > buffer,
    ...
    _buffer(buffer),
    _snr(new SharedType<double>(0.0)),
	_x(windowSize), 
...
void AMC::FeatureExtractor::setSnr(boost::shared_ptr<SharedType<double> > snr)
{
    _snr.swap(snr);
}
...
{
    AMC::ModType tempModType;
    boost::unique_lock<boost::shared_mutex> modTypeLock(*_sharedModType->getMutex(), boost::defer_lock);
    boost::unique_lock<boost::shared_mutex> snrLock(*_snr->getMutex(), boost::defer_lock);
    while (_isExtracting)
    {
        if(get_x())
        {
            ...
            case AMC::FeatureExtractor::WRITE_TO_FILE:
                ...
                tempModType = getSlowModType();

                snrLock.lock();
                _fileWriter.writeToFile(_snr->getData(), (tempModType == _writerModType));
                snrLock.unlock();

                modTypeLock.lock();
                _sharedModType->getData() = tempModType;
                modTypeLock.unlock();
                break;
